...
class PlaygroundParticleSystemInspectorC : Editor {
	...
	public static void SetWireframeVisibility () {
        ...
        EditorUtility.SetSelectedRenderState(playgroundParticlesScriptReference.particleSystemRenderer, PlaygroundC.reference.drawWireframe ? EditorSelectedRenderState.Wireframe : EditorSelectedRenderState.Highlight);
		...
		if (playgroundSettings.particlesFoldout) {
			...
			if (playgroundSettings.renderingFoldout) {
				...
				if (playgroundSettings.textureSheetAnimationFoldout) {
					shuriken.UpdateIfRequiredOrScript();
			...
			// Advanced Settings
			string localSimulationSpaceName = playgroundParticlesScriptReference.GetComponent<ParticleSystem>().main.simulationSpace==ParticleSystemSimulationSpace.Local?playgroundLanguage.localSpace:playgroundLanguage.globalSpace;
			...
			if (playgroundSettings.advancedFoldout) {
				...
				GUILayout.Label (playgroundParticlesScriptReference.GetComponent<ParticleSystem>().main.simulationSpace.ToString(), EditorStyles.miniLabel, GUILayout.ExpandWidth(false));
				GUILayout.EndHorizontal();
				if (playgroundSettings.advancedSimulationFoldout) {
					GUI.enabled = (playgroundParticlesScriptReference.source!=SOURCEC.Projection);
                    ParticleSystem.MainModule main = playgroundParticlesScriptReference.GetComponent<ParticleSystem>().main;
					main.simulationSpace = (ParticleSystemSimulationSpace)EditorGUILayout.EnumPopup(playgroundLanguage.simulationSpace, main.simulationSpace);
					GUI.enabled = true;
					if (main.simulationSpace==ParticleSystemSimulationSpace.Local && playgroundParticlesScriptReference.source!=SOURCEC.Projection) {
	...
	void OnSceneGUI () {
		...
		if (playgroundScriptReference.drawGizmos && playgroundSettings.particleSettingsFoldout && (playgroundParticlesScriptReference.sorting==SORTINGC.NearestNeighbor || playgroundParticlesScriptReference.sorting==SORTINGC.NearestNeighborReversed) && playgroundParticlesScriptReference.playgroundCache!=null && playgroundParticlesScriptReference.playgroundCache.targetPosition!=null && playgroundParticlesScriptReference.playgroundCache.targetPosition.Length>0 && playgroundParticlesScriptReference.particleCount>0) {
			...
			switch (playgroundParticlesScriptReference.nearestNeighborOriginMethod) {
			case NEARESTNEIGHBORORIGINMETHOD.SourcePoint:
				sortOriginPosition = playgroundParticlesScriptReference.shurikenParticleSystem.main.simulationSpace==ParticleSystemSimulationSpace.World?
