	...
	public class PlaygroundParticlesC : MonoBehaviour {
		...
		*************************************************************************************************************************************************/
		...
		public static void SetRotationRandom (PlaygroundParticlesC playgroundParticles, float rotationMinimum, float rotationMaximum) {
			if( playgroundParticles.playgroundCache!=null && playgroundParticles.particleCache!=null )
			{
		...
		{
			if( playgroundParticles!=null && playgroundParticles.playgroundCache!=null && playgroundParticles.playgroundCache.previousTargetPosition!=null && playgroundParticles.playgroundCache.targetPosition!=null )
			{
				for( int p = 0; p < playgroundParticles.particleCache.Length; p++ )
				{
				...
				ParticleSystem partSyst = playgroundParticles.GetComponent<ParticleSystem>();
				if( refreshParticleSystem && partSyst != null )
					partSyst.SetParticles(playgroundParticles.particleCache, playgroundParticles.particleCache.Length);
			}
		}
		...
		public bool PrepareThreadedCalculations () {
			...
			localSpace = (shurikenParticleSystem.main.simulationSpace == ParticleSystemSimulationSpace.Local);
			...
			if (emit) {
				...
				case SOURCEC.Transform:
					...
					if (isDoneThread) {
						stRot = sourceTransform!=null?sourceTransform.rotation:Quaternion.identity;
					}
				...
				case SOURCEC.Projection:
					if (projection.projectionTexture!=null && projection.projectionTransform!=null) {
						...
						if (localSpace)
                        {
                            ParticleSystem.MainModule main = shurikenParticleSystem.main;
			...
			if (isReadyForThreadedCalculations) {
				...
				if (sorting==SORTINGC.NearestNeighbor||sorting==SORTINGC.NearestNeighborReversed) {
					if (nearestNeighborOriginMethod==NEARESTNEIGHBORORIGINMETHOD.Transform && nearestNeighborOriginTransform!=null) {
						nearestNeighborOriginTransformPosition = shurikenParticleSystem.main.simulationSpace==ParticleSystemSimulationSpace.World?
