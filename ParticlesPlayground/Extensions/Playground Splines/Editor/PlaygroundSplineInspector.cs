...
public class PlaygroundSplineInspector : Editor {
	...
	public override void OnInspectorGUI () {
		...
		if (playgroundSettings.playgroundSplineFoldout) {
			...
			bool bLocalScale = EditorGUILayout.Toggle(playgroundLanguage.localScale, spline.LocalScale);
			if( spline.LocalScale!=bLocalScale )
			{
				EditorUtility.SetDirty(spline);
				spline.LocalScale = bLocalScale;
			}

			bool bLocalRotation = EditorGUILayout.Toggle(playgroundLanguage.localRotation, spline.LocalRotation);
			if( spline.LocalRotation!=bLocalRotation )
			{
				EditorUtility.SetDirty(spline);
				spline.LocalRotation = bLocalRotation;
			}

			bool bLocalPosition = EditorGUILayout.Toggle(playgroundLanguage.localPosition, spline.LocalPosition);
			if( spline.LocalPosition!=bLocalPosition )
			{
				EditorUtility.SetDirty(spline);
				spline.LocalPosition = bLocalPosition;
			}
			// BIGPOINT OUT
