...
namespace PlaygroundSplines {
	...
	public class PlaygroundSpline : MonoBehaviour {
		...
		private bool loop;
		/// <summary>
		/// BIGPOINT - Use local Scale
		/// </summary>
		[SerializeField]
		private bool bLocalScale;
		/// <summary>
		/// BIGPOINT - Use Local rotation
		/// </summary>
		[SerializeField]
		private bool bLocalRotation;
		/// <summary>
		/// BIGPOINT - Use Local position
		/// </summary>
		[SerializeField]
		private bool bLocalPosition;
		...
		/// <summary>
		/// BIGPOINT - Gets or sets a value if we use lossyscale
		/// </summary>
		/// <value><c>true</c> if set to loop; otherwise, <c>false</c>.</value>
		public bool LocalScale
		{
			get { return bLocalScale; }
			set { bLocalScale = value; }
		}

		/// <summary>
		/// BIGPOINT - Gets or sets a value if we use local rotation
		/// </summary>
		/// <value><c>true</c> if set to loop; otherwise, <c>false</c>.</value>
		public bool LocalRotation
		{
			get { return bLocalRotation; }
			set { bLocalRotation = value; }
		}

		/// <summary>
		/// BIGPOINT - Gets or sets a value if we use local position
		/// </summary>
		/// <value><c>true</c> if set to loop; otherwise, <c>false</c>.</value>
		public bool LocalPosition
		{
			get { return bLocalPosition; }
			set { bLocalPosition = value; }
		}
		...
		public Vector3 GetPoint (float t) {
			...
			if( reverse )
			{
				t = 1f - t;
				t = t - timeOffset;
				// PATCH BIGPOINT : Don't return to 0 if not in loop
				if( loop ) t = t % 1f;
				if( t < 0 )
					t = 1f + t;
			}
		...
		*************************************************************************************************************************************************/
		...
		void SetMatrix ()
		{
			Vector3 v3Scale = bLocalScale ? splineTransform.localScale : splineTransform.lossyScale;
			Quaternion qRotation = bLocalRotation? splineTransform.localRotation : splineTransform.rotation;
			Vector3 v3Position = bLocalPosition ? splineTransform.localPosition : splineTransform.position;
			if( previousPosition!=v3Position || previousRotation!=qRotation || previousScale!=v3Scale )
				splineTransformMx.SetTRS( v3Position, qRotation, v3Scale );
			previousPosition = v3Position;
			previousRotation = qRotation;
			previousScale = v3Scale;
			isReady = true;
		}
	}
